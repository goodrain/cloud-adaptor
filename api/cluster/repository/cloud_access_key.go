...
func NewCloudAccessKeyRepo(db *gorm.DB) cluster.CloudAccesskeyRepository {
	return &CloudAccessKeyRepo{DB: db}
}

//Create create, Keep an enterprise with the same provider have one accesskey
func (c *CloudAccessKeyRepo) Create(ck *models.CloudAccessKey) error {
	var old models.CloudAccessKey
...
func (c *CloudAccessKeyRepo) GetByProviderAndEnterprise(providerName, eid string) (*models.CloudAccessKey, error) {
	var old models.CloudAccessKey
